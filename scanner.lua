...
local catchButtonPaths = {
	{ "Frame", "Frame", "Frame", "Frame", "TextButton" },
...
local function isCatchButtonText(button)
	local text = cleanButtonText(getDeepText(button))

	return isCatchText(text)
		or text:find("catch", 1, true) ~= nil
		or text:find("captur", 1, true) ~= nil
end

local function isScannerPathCatchButton(button)
	if not button or not button:IsA("TextButton") or not isGuiVisible(button) then
		return false
	end

	if not isCatchButtonText(button) then
		return false
	end

	local fullName = cleanButtonText(safeFullName(button))

	return fullName:find("screengui.frame.frame.frame.frame.textbutton", 1, true) ~= nil
		or hasReleaseSibling(button)
end
...
local function scoreCatchButton(button, boost)
	...
	local textBlob = cleanButtonText(getDeepText(button))
	local buttonBlob = cleanButtonText(textBlob .. " " .. tostring(button.Name or ""))
	...
	if isCatchText(textBlob) then score += 620 end
	...
	if fullBlob:find("screengui.frame.frame.frame.frame.textbutton", 1, true) then
		score += 420
	end
...
local function collectCatchCandidates(includeGlobal)
	...
	for _, rootGui in ipairs(playerGui:GetChildren()) do
		for _, path in ipairs(catchButtonPaths) do
			local button = getPath(rootGui, path)
			pushCatchCandidate(candidates, button, isScannerPathCatchButton(button) and 900 or 190, "playergui_frame_path")
		end
	end
	...
	if includeGlobal then
		for _, obj in ipairs(playerGui:GetDescendants()) do
			if obj:IsA("TextButton") then
				pushCatchCandidate(candidates, obj, isScannerPathCatchButton(obj) and 700 or 0, "global_scan")
			end
		end
	end
...
local function findExactScannerCatchButton()
	for _, rootGui in ipairs(playerGui:GetChildren()) do
		for _, path in ipairs(catchButtonPaths) do
			local button = getPath(rootGui, path)

			if isScannerPathCatchButton(button) then
				return button
			end
		end
	end
	...
	for _, obj in ipairs(playerGui:GetDescendants()) do
		if isScannerPathCatchButton(obj) then
			return obj
		end
	end
...
local function clickCatchButton()
	...
	local exactButton = findExactScannerCatchButton()
	if exactButton then
