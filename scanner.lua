...
_spawn(function()
_wait(1.1)
local __ok,__err=_pcall(function()
local Players = game:GetService("Players")
local UserInputService = game:GetService("UserInputService")
local ReplicatedStorage = game:GetService("ReplicatedStorage")
local LocalPlayer = Players.LocalPlayer
...
_pcall(function()
local __env=_G
if type(getgenv)=="function" then local __g=getgenv() if type(__g)=="table" then __env=__g end end
__env.__REF_EMBEDDED_STAB_PATCH_ERROR=tostring(__err)
end)
end
end)
