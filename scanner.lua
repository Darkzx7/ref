...
-- v65: v54 stable + embedded stab patch loaded safely from string
...
-- ─────────────────────────────────────────────────────────────────────────────
-- SAFE EMBEDDED STAB HITBOX LOADER
-- The Stab Hitbox patch is embedded as text and compiled safely.
-- This avoids contaminating the main Ref boot and avoids nil-call crashes if compile fails.
-- ─────────────────────────────────────────────────────────────────────────────

do
    local __REF_STAB_PATCH_SOURCE = [====[
...
Patch.Status = "Loaded"
    ...
    local function __ref_get_env_table()
        local env = _G
        pcall(function()
            if type(getgenv) == "function" then
                local got = getgenv()
    ...
    local function __ref_load_stab_patch_safely()
        local env = __ref_get_env_table()
        if type(env) == "table" and env.__REF_STAB_EMBEDDED_LOADED == true then
            return true
        end
        ...
        if type(loader) ~= "function" then
            if type(env) == "table" then
                env.__REF_STAB_EMBEDDED_ERROR = "loadstring/load unavailable"
            end
            return false
        end

        local okCompile, compiledOrErr = pcall(loader, __REF_STAB_PATCH_SOURCE)
        if not okCompile then
            if type(env) == "table" then
                env.__REF_STAB_EMBEDDED_ERROR = "compile pcall failed: " .. tostring(compiledOrErr)
            end
            return false
        end

        if type(compiledOrErr) ~= "function" then
            if type(env) == "table" then
                env.__REF_STAB_EMBEDDED_ERROR = "compile returned nil/non-function: " .. tostring(compiledOrErr)
            end
            return false
        end
        ...
        if not okRun then
            if type(env) == "table" then
                env.__REF_STAB_EMBEDDED_ERROR = "runtime failed: " .. tostring(runErr)
            end
            return false
        end

        if type(env) == "table" then
            env.__REF_STAB_EMBEDDED_ERROR = nil
            env.__REF_STAB_EMBEDDED_LOADED = true
        end

        return true
    end

    local function __ref_start_later()
        if type(task) == "table" and type(task.wait) == "function" then
            task.wait(1.5)
        elseif type(wait) == "function" then
            wait(1.5)
        end
        pcall(__ref_load_stab_patch_safely)
    end

    if type(task) == "table" and type(task.spawn) == "function" then
        task.spawn(__ref_start_later)
    elseif type(spawn) == "function" then
        spawn(__ref_start_later)
    else
        pcall(__ref_load_stab_patch_safely)
    end
end
