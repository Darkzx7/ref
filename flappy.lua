...
task.spawn(function()
	...
	local function buildCorridor(col, bRect)
		...
		if effH < bRect.height + 10 then return nil end

		local halfB  = bRect.height * .5
		local margin = halfB + SAFE_MARGIN
		if margin * 2 >= effH then margin = (effH - bRect.height) * .5 - 1.0 end
	...
	local function computeTarget(corridors)
		...
		if c2 then
			...
			local bias = 0.5
			if     d >  35 then bias = 0.72
			elseif d >  18 then bias = 0.63
			elseif d < -35 then bias = 0.28
			elseif d < -18 then bias = 0.37
			end
			...
			elseif dl <  8  then prep = 0.92
			elseif dl <  22 then prep = 0.78
			elseif dl <  42 then prep = 0.58
			elseif dl <  70 then prep = 0.35
			elseif dl < 110 then prep = 0.16
			end
	...
	local MIN_INTERVAL  = 0.078
	local EMRG_INTERVAL = 0.040
	...
	RunService.Heartbeat:Connect(function()
		...
		if lastY then
			velY       = (b.cy - lastY) / dt
			smoothVelY = smoothVelY * 0.38 + velY * 0.62
		end
		lastY    = b.cy
		lastTime = now
		...
		if #corridors == 0 then
			...
			table.insert(corridors, {
				...
				safeTop=aT+22, safeBottom=aB-34,
				center=midY,
				minCY=aT+b.height*.5+8,
				maxCY=aB-b.height*.5-20,
			})
		end
		...
		elseif c1.distLeft < 42 then dz = 4.0
		end
		...
		if c2 and not emergency then
			if c2.center > c1.center + 16 and err < 3 and smoothVelY < 8 then
				shouldTap = false
				noTapUntil = math.max(noTapUntil, now + 0.058)
			end
		end
		...
		if c1.distLeft < b.width * 2.8 then
			...
			if b.bottom >= c1.effBottom - 6 or predBot >= c1.effBottom - 8 then
				shouldTap = true
				emergency = true
			end
		end
